...
[ $# -ne 3 ] && echo "ex) $0 [db_name] [tbl_name] [inc_day]"
...
mysql -uroot ${db_name} -Nse "SHOW CREATE TABLE ${tbl_name}\G" > ${create_ddl}
...
mysql -uroot ${db_name} -vvv < "${add_sql}" > ${add_log}
```

```sh:02_drop_partition.sh
#!/bin/sh

[ $# -ne 2 ] && echo "ex) $0 [db_name] [tbl_name]"
...
cat ${drop_sql}
mysql -uroot ${db_name} -vvv < "${drop_sql}" > ${drop_log}
```
